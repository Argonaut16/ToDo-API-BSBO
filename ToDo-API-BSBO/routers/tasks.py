...
async def create_task(task: TaskCreate) -> TaskResponse:
    ...
    # FastAPI автоматически преобразует dict → TaskResponse
    return new_task
...
async def update_task(task_id: int, task_update: TaskUpdate) -> TaskResponse:
    ...
    # ШАГ 2: обновляем только переданные поля
    update_data = task_update.model_dump(exclude_unset=True)
    for field, value in update_data.items():
        task[field] = value

    # ШАГ 3: пересчитываем квадрант при изменении важности или срочности
